...
async def _show_calc_type_page(message_or_event, page: int):
    ...
    # VK ограничивает 10 строк: 3 типа + навигация + отмена = 5 строк (безопасно)
    types_per_page = 3
...
async def _finalize_lead(peer_id: int, user_id: int, data: dict, comment: str):
    ...
    if DB_TYPE == "postgresql":
        cur.execute(
            f"INSERT INTO leads (user_id, name, phone, address, comment, calc_data, status, created_at, platform) "
            f"VALUES ({ph}, {ph}, {ph}, {ph}, {ph}, {ph}, 'Новая', {ph}, 'vk') RETURNING id",
        ...
        cur.execute(
            f"INSERT INTO leads (user_id, name, phone, address, comment, calc_data, status, created_at, platform) "
            f"VALUES ({ph}, {ph}, {ph}, {ph}, {ph}, {ph}, 'Новая', {ph}, 'vk')",
            (user_id, name, phone, address, comment, calc_data, datetime.now().isoformat()),
        )
...
async def review_text_handler(message: Message):
    ...
    ph = _placeholder()
    cur.execute(
        f"INSERT INTO reviews (author, text, created_at, approved) VALUES ({ph}, {ph}, {ph}, 1)",
...
async def cmd_types(event: MessageEvent, payload: dict = None):
    ...
    # 3 типа + навигация (1 строка) + главное меню (1 строка) = 5 строк (безопасно)
    types_per_page = 3
    total = len(types)
...
async def cmd_admin_prices(event: MessageEvent, payload: dict = None):
    ...
    # VK ограничивает 10 строк: 3 цены + навигация + админка = 5 строк (безопасно)
    prices_per_page = 3
    total = len(types_list)
